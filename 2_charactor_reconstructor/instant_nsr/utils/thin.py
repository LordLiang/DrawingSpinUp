...
def thining_processing(v, f, config, save_cache=True, theta_1=11, theta_2=9, r=11):
    if save_cache:
        cache_dir = os.path.join(config.output_dir, 'thin')
        os.makedirs(cache_dir, exist_ok=True)

    mask_fn = os.path.join(config.input_dir, 'mask/front.png')
    ...
    offset_values, offset_mask = get_offset_mask(v, f, thin_coords, coord_dists, min_thickness, config.thin_type)
