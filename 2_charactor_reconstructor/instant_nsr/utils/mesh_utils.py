...
def load_color(data_dir, view):
    if view == 'front':
        color_filepath = os.path.join(data_dir, '../char/ffc_resnet_inpainted_0.png')
        color = Image.open(color_filepath).resize((1024, 1024), Image.LANCZOS)
        color = np.array(color, dtype=np.float32) / 255.
        mask_filepath = os.path.join(data_dir, 'mask', '%s.png'%(view))
        mask = np.array(Image.open(mask_filepath), dtype=np.float32) / 255.
...
def color_projection(vertices, faces, data_dir):
    ...
    ray_directions = [(0,0,1), (0,0,-1), (-1,0,0), (1,0,0)]
    ...
    for j in range(len(views)):
        ...
        if len(valid_indices) > 0:
            color = load_color(data_dir, view = views[j])
            valid_color = get_color_from_image(vertices[valid_indices], color, rotation_angles[j])
            ...
            for i in range(len(valid_indices)):
                ind = valid_indices[i]
                result = mesh_raycast.raycast(vertices[ind], ray_directions[j], mesh=triangles)
                if len(result) > 0:
