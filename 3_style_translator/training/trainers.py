...
class Trainer(object):
    def __init__(self,
                 ...
                 use_mask, use_pos, use_edge, device
                 ):
        
        training_dataset = DatasetPatches_M(os.path.join(data_root, 'rest_pose'),
                                            trainer_config['pre_dir'], trainer_config['post_dir'], trainer_config['post_name'],
                                            trainer_config['patch_size'], use_mask, use_pos, use_edge)
        ...
        self.pre_dir = trainer_config['pre_dir']
    ...
    def test_on_full_image(self, generator, result_folder, save_alpha=True):
        for test_name in self.testing_name_list:
            data_root = os.path.join(self.data_root, test_name)
            dataset = DatasetFullImages(data_root, self.pre_dir, self.use_mask, self.use_pos, self.use_edge)
